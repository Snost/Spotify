...
export function BottomNav({ active }: { active: 'home' | 'search' | 'playlists' | 'library' | 'profile' }) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 border-t border-white/5 bg-groov-bg/90 backdrop-blur lg:hidden">
...
function NavItem({ icon, label, active }: { icon: React.ReactNode; label: string; active: boolean }) {
  return (
    <button className="flex flex-col items-center gap-1">
      <div className={active ? 'text-groov-accent' : 'text-groov-secondary'}>{icon}</div>
      <div className={`text-[10px] ${active ? 'text-groov-accent' : 'text-groov-secondary'}`}>{label}</div>
    </button>
  )
}
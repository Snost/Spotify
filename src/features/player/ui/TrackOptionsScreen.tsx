...
export function TrackOptionsScreen() {
  ...
  const artistLabel = useMemo(() => {
    if (!currentTrack?.mainArtists?.length) return 'Unknown artist'
  ...
  return (
    <div className="min-h-[100dvh] bg-groov-bg px-[24px] pb-[34px] pt-[18px] text-groov-accent">
      <div className="mx-auto w-full max-w-[402px]">
        <div className="flex items-center justify-between pb-[22px]">
          <div className="text-[22px] font-semibold leading-none opacity-0">
            .
          </div>
          <button
            type="button"
            onClick={handleDismiss}
            className="text-[16px] font-medium leading-none text-groov-accent"
          >
            Готово
          </button>
        </div>
          ...
          {menuItems.map((item, index) => (
            <div key={item.key}>
              <button
                ...
                className={[
                  'flex w-full items-center gap-[14px] py-[11px] text-left',
                  item.danger ? 'text-groov-accent' : 'text-groov-accent',
                ].join(' ')}
              >
        ...
        <button
          type="button"
          onClick={handleDismiss}
          className="mt-[22px] h-[40px] w-full rounded-[14px] bg-groov-primary text-[16px] font-semibold leading-none text-groov-accent"
        >
          Відхилити
        </button>

        <div className="mx-auto mt-[16px] h-[5px] w-[134px] rounded-full bg-groov-accent" />
      </div>
    </div>
  )
}
